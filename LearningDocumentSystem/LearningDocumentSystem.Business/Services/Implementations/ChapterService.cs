...
namespace LearningDocumentSystem.Business.Services.Implementations
{
    public class ChapterService : IChapterService
    {
        ...
        public ChapterService(IUnitOfWork uow, IMapper mapper, ILogger<ChapterService> logger, INotificationService notificationService)
        {
            _uow = uow;
            _mapper = mapper;
            _logger = logger;
        ...
        public async Task<ChapterDto> CreateAsync(CreateChapterDto dto)
        {
            ...
            await _notificationService.SendNotificationAsync("ChapterCreated", result);
            return result;
        }
        ...
        public async Task<ChapterDto> UpdateAsync(UpdateChapterDto dto)
        {
            ...
            var result = _mapper.Map<ChapterDto>(chapter);
            await _notificationService.SendNotificationAsync("ChapterUpdated", result);
            return result;
        }
        ...
        public async Task DeleteAsync(int id)
        {
            ...
            await _uow.SaveChangesAsync();
            await _notificationService.SendNotificationAsync("ChapterDeleted", new { chapterId = id });
        }
    }
}